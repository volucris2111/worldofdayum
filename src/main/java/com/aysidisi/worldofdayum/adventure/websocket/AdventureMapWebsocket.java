...
public class AdventureMapWebsocket
{
	...
	public void movement(final Integer key, final SimpMessageHeaderAccessor headerAccessor)
	{
		...
		HashSet<ObjectId> avatars = new HashSet<ObjectId>();
		...
		if (avatar != null)
		{
			...
			if (dungeonFieldMovedTo != null && this.fieldTypeService
					.findById(dungeonFieldMovedTo.getFieldTypeId()).getWalkable())
			{
	...
	public void updateAvatars(final HashSet<ObjectId> avatarIdsToUpdate)
	{
				...
				.getWebSocketSessionCacheBySubject("/user/adventure/updateavatars").values())
		{
			if (avatarIdsToUpdate.contains(account.getCurrentAvatarId()))
			{
				this.messagingTemplate.convertAndSendToUser(account.getName(),
						"/adventure/updateavatars", this.avatarService.getAvatarPojosNearAvatar(
								this.avatarService.findOne(account.getCurrentAvatarId())));
			}
		}
	}
